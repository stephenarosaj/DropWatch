...
/**
 * Record to hold information on an Album Note that when one of these objects is returned via
 * search, all fields are present (EXCEPT FOR "tracks", since the object returned there is
 * "SimplifiedAlbumObject"!) REFERENCE:
 ...
 * @param genres "A list of the genres the album is associated with. If not yet classified, the
 *     array is empty."
 ...
 * @param release_date_precision "The precision with which release_date value is known." Allowed
 *     values: "year", "month", or "day"
 * @param restrictions "Included in the response when a content restriction is applied." Allowed
 *     values:"market - The content item is not available in the given market. "product" - The
 *     content item is not available for the user's subscription type. "explicit" - The content item
 *     is explicit and the user's account is set to not play explicit content.
 * @param album_type "The type of the album." - USED ONLY IN /search Allowed values: "album",
 *     "single", "compilation"
 * @param album_group "The field is present when getting an artist's albums. Compare to album_type
 *     this field represents relationship between the artist and the album." - USED ONLY IN
 *     /artists/{id}/albums Allowed values: "album", "single", "compilation", "appears_on"
